...
router.post('/login', async (req, res) => {
  ...
  const userQuery = await pool.query('SELECT * FROM users WHERE username = $1 OR email = $1', [identifier]);
  ...
  const tokenJWT = jwt.sign({ id: user.id, username: user.username, email: user.email }, process.env.JWT_SECRET, { expiresIn: '1h' });
  const Option = {
    maxAge: 3600000,
    httpOnly: true,
   };
